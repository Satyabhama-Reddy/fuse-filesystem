#include "head.h"
void change_mod_time(int file)
{
	fs.inodes[file].mod_t=time(NULL);
}
void change_cha_time(int file)
{
	fs.inodes[file].cha_t=time(NULL);
}
void change_acc_time(int file)
{
	fs.inodes[file].acc_t=time(NULL);
}
void disk_save()
{
	FILE *fd;
	fd=fopen("/home/satya/Desktop/project/bin.bin","wb+");
	fseek(fd,0,SEEK_SET);
...
static void *file_init(struct fuse_conn_info *conn)
{
	printf("INIT\n");
	ffss=&fs;
	memset(ffss,0,sizeof(fs));
	FILE *fd=fopen("/home/satya/Desktop/project/bin.bin","rb+");
	fseek(fd,0,SEEK_END);
	long int p=ftell(fd);
	if(p!=0)
	{
...
{	
	...
	stbuf->st_ctime=fs.inodes[file].cha_t;
	}
	else 
		return -ENOENT;
	return 0;
...
static int file_mkdir(const char *path, mode_t mode)
{
		...
		fs.inodes[par].link=fs.inodes[par].link+1;
		change_mod_time(par);
		change_cha_time(par);
	disk_save();
	return 0;
...
int file_read(const char * path, char * buf, size_t size, off_t off,struct fuse_file_info * fi)
{
	...
	if(file==-1)
		return -ENOENT;
	if(!(fs.inodes[file].mode & S_IRUSR))
		return -EACCES;
...
int file_write(const char * path, const char * buf, size_t size, off_t off,struct fuse_file_info * fi)
{
	...
	if(file==-1)
		return -ENOENT;
	if(!(fs.inodes[file].mode & S_IWUSR))
		return -EACCES;
	int block=fs.inodes[file].db;
	...
	while(size>0)
	{
 	...
 	printf("cnt:%d\n",cnt);
 	printf("file size:%d\n",fs.inodes[file].size);
 	change_mod_time(file);
 	change_cha_time(file);
 	disk_save();
 	return cnt;
}
...
int file_rmdir(const char * path)
{
	...
	fs.sup.imap[file]=0;
	fs.inodes[parent].link=fs.inodes[parent].link-1;
	change_mod_time(parent);
	change_cha_time(parent);
	disk_save();
	return 0;
}
...
int file_utime(const char * path, struct utimbuf *buf)
{
	...
	fs.inodes[file].cha_t=buf->actime;
...
int file_chmod(const char * path , mode_t m, struct fuse_file_info *fi)
{
	int parent;
	int file=traverse_to(path,&parent);
	if(file==-1)
		return -ENOENT;
	fs.inodes[file].mode=m;
	change_cha_time(file);
	disk_save();
	return 0;
}
